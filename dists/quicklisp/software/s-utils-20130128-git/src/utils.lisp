...
(defun copy-stream (in out &optional (buffer (make-string 4096)) second-buffer (convertor #'identity))
  ...
  ;; in and out can be functions too, reading into a buffer, writing from a buffer until end
  (if second-buffer
      (labels ((convert-buffer (limit)
                 (loop :for i :below limit 
                       :do (setf (elt second-buffer i) (funcall convertor (elt buffer i)))))
               (copy-chunks ()
                 (let ((size (if (functionp in) 
                                 (funcall in buffer) 
                               (read-sequence buffer in))))
                   (unless (zerop size)
                     (convert-buffer size)
                     (if (functionp out)
                         (funcall out second-buffer size)
...
(defun format-duration (seconds &key (unit-names +en-duration-unit-names+) stream)
  ...
  (let ((out (or stream (make-string-output-stream)))
    ...
    (cond ((plusp seconds)
          ...
          (t (format out "~d ~a~p" seconds (aref unit-names 4) seconds)))
    (unless stream
      (get-output-stream-string out))))
